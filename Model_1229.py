...
def __get_train_op(model_name, learning_rate=0.001, is_freeze=False):
    # Trainning Op Set
    train_op = None
    ...
    with tf.control_dependencies(update_ops):
        if is_freeze:
            train_op = tf.train.AdamOptimizer(learning_rate).minimize(loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope="Map_Layers"))
        else:
...
def build_model(model_name):
    ...
    with tf.name_scope(model_name):
        ...
        #OPs
        # add AUG
        # X_place = tf.cond(is_training, lambda : tf.image.random_flip_left_right(tf.image.random_flip_up_down(X_place)), lambda : X_place) 

        with tf.variable_scope('Feature_Layers'):
            # input BN
        ...
        with tf.variable_scope('Map_Layers'):
...
def training(train_set, valid_set, model_name, epochs=10, learning_rate=0.001, early_stop=None, restore_name=None, is_shuffle=False, is_freeze=False):

    start_time = time.time()
    # Trainning Op Set
    train_op = __get_train_op(model_name, learning_rate, is_freeze)
    min_loss = np.inf
    min_epoch = -1
